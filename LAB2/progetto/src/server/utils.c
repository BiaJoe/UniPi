...
void mallocate_emergency_type(
	...
	if(!emergency_types[i]->emergency_desc) {
		perror("Errore allocazione memoria emergency_type_name ");
		exit(EXIT_FAILURE);
	}
...
void free_emergency_types(emergency_type_t **emergency_types){
	for(int i = 0; emergency_types[i] != NULL; i++){
		...
		// libero ogni rescuer_request
		for(int j = 0; j < emergency_types[i]->rescuers_req_number; j++)
			free(emergency_types[i]->rescuers[j]);

		// libero l'array di puntatori ai rescuer_requests
